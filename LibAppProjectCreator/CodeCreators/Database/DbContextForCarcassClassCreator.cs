    ...
    public override void CreateFileStructure()
    {
        var block = new CodeBlock(string.Empty, new OneLineComment($"Created by {GetType().Name} at {DateTime.Now}"),
            ...
            new CodeBlock($"public sealed class {_projectNamespace}DbContext : CarcassDbContext", string.Empty,
                ...
                string.Empty, new OneLineComment("ბაზაში არსებული ცხრილები წარმოდგენილი DbSet-ების სახით"),
                new OneLineComment(
                ...
                new CodeBlock("protected override void OnModelCreating(ModelBuilder modelBuilder)",
                    new OneLineComment(
                        $"Console.WriteLine(\"{_projectNamespace}DbContext OnModelCreating Start...\");"), string.Empty,
                    "base.OnModelCreating(modelBuilder)", string.Empty,
                    new OneLineComment(
                        $"Console.WriteLine(\"{_projectNamespace}DbContext OnModelCreating Pass 1...\");"),
                    string.Empty, "modelBuilder.ApplyConfigurationsFromAssembly(AssemblyReference.Assembly)",
                    string.Empty)));
