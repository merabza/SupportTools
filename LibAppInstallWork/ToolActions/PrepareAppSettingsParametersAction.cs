...
public sealed class PrepareAppSettingsParametersAction : ToolAction
{
    ...
    public PrepareAppSettingsParametersAction(ILogger logger, string sourceJsonFileName) : base(logger,
        "Prepare AppSettings Parameters", null, null)
    {
        _logger = logger;
    ...
    public string? PreparedAppSettingsJsonContent { get; private set; }
    public JObject? AppSetJObject { get; private set; }

    protected override async ValueTask<bool> RunAction(CancellationToken cancellationToken = default)
    {
        PreparedAppSettingsJsonContent = await CreateAppSettingsJson(cancellationToken);
        //bool success = false;
        //if (AppSettingsJsonContent != null)
        //{
        //    await File.WriteAllTextAsync(_sourceJsonFileName, AppSettingsJsonContent, cancellationToken);
        //    success = true;
        //}
    ...
    private async Task<string?> CreateAppSettingsJson(CancellationToken cancellationToken = default)
    {
        ...
        string appSetJson = await File.ReadAllTextAsync(_sourceJsonFileName, cancellationToken);
        AppSetJObject = JObject.Parse(appSetJson);
        ...
        if (directoryName != null)
        {
            ...
            if (csprojFile != null)
            {
                ...
                {
                    string secretJson = await File.ReadAllTextAsync(userSecretContentFileName, cancellationToken);
